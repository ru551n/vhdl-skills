...
Project mode shells out to the project's own build script, which needs the
*project's* dependencies (``tsfpga``, ``hdl-registers``, ...) — never this
...
* the subprocess environment is *activated* against that venv —
  ...
  nested ``python``/``pip`` the build script itself invokes resolve inside
...
``TSFPGA_MCP_PROJECT_PYTHON`` names an interpreter explicitly, or when
``TSFPGA_MCP_PROJECT_AUTO_VENV`` is falsy. It is a no-op — with an
explanatory note for ``vhdl-tools synth project-status`` — when the project declares
no dependencies or ``uv`` is not installed; the PATH interpreter is then
used as before.

This file is kept byte-identical to vunit-mcp's ``project_venv`` apart from
this docstring: an agent runs both servers against one project and they
share the provisioning lock, so fixes must be ported to both.
"""
...
def ensure_venv(
    ...
    *,
    ...
    that skipped the lock would hand back an empty environment and build_fpga.py
    would fail with ``No module named 'tsfpga'``. Taking the lock first is
