...
from vhdl_tools.vunit import server
from vhdl_tools.vunit.models import GetTestWaveformInput, RunTestsInput
from vhdl_tools.vunit.server import vunit_get_test_waveform, vunit_run_tests
...
# vhdl-tools' own venv gets stock vunit-hdl (a tsfpga dependency), whose NVC
# driver has no --wave: these tests need the fork (upstream PR #1101).
_HAS_WAVE = "wave" in __import__("inspect").signature(NVCInterface.__init__).parameters
pytestmark = [
    pytest.mark.skipif(NVC is None, reason="requires nvc on PATH"),
    pytest.mark.skipif(not _HAS_WAVE, reason="requires a VUnit with --wave (PR #1101)"),
]
