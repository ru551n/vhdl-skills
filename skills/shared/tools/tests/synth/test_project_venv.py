...
from vhdl_tools.synth.project_venv import (
    _provisioning_lock,
    activate,
    auto_venv_enabled,
    create_venv,
...
def test_provisioning_lock_is_per_project(tmp_path):
    ...
    from vhdl_tools.synth.project_venv import lock_path
...
def test_discovery_waits_for_a_peer_that_is_still_installing(tmp_path):
    ...
    taking the lock therefore handed back an environment with no tsfpga in
    it, and build_fpga.py died with ``No module named 'tsfpga'``.
    """
    (tmp_path / "requirements.txt").write_text("", encoding="utf-8")
